...
If the child agent throws an error, it's caught and returned as a tool message to the parent. This lets the parent recover gracefully:

```ts
// If the child fails, the parent sees:
