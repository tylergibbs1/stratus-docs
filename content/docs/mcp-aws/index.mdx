...
When a client first connects, it only sees the `always` tier plus `search_tools`. The agent uses `search_tools` to discover relevant capabilities, and the server promotes matching tools into the visible set for that session. After promotion, the server emits `tools/list_changed`, the client refreshes `tools/list`, and the agent can call the newly visible tools directly.
...
<Callout>
Looking for the agent SDK? See [Getting Started](/getting-started) for `@usestratus/sdk`.
</Callout>
