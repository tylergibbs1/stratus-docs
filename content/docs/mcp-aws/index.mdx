---
title: "@stratus/mcp-aws"
description: Build, deploy, and manage MCP servers on AWS in TypeScript
---

`@stratus/mcp-aws` is a TypeScript library for building MCP servers that deploy to AWS. Progressive disclosure, tool gating, code mode, and one-line Lambda deploys.

<Callout type="info">
`@stratus/mcp-aws` wraps the official `@modelcontextprotocol/sdk` — it adds AWS deployment, auth, gating, progressive disclosure, and observability on top of the standard MCP protocol.
</Callout>
...
```bash tab="bun"
bun add @stratus/mcp-aws zod@4
```

```bash tab="npm"
npm install @stratus/mcp-aws zod@4
```
