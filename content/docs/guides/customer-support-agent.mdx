...
Route customer requests to specialized agents with tools, audit every handoff with hooks, and enforce guardrails on input. This guide walks you through a production-ready triage system from scratch.

You will learn how to:

- **Route with handoffs** -- build a triage agent that transfers customers to the right specialist
- **Add domain tools** -- give each specialist agent tools for order lookup, refund processing, and more
- **Audit with hooks** -- log every handoff, tool call, and resolution for compliance
- **Enforce guardrails** -- block toxic input before it reaches your agents
- **Control permissions** -- deny high-value refunds that need manager approval
- **Persist conversations** -- save and resume support sessions across shifts
