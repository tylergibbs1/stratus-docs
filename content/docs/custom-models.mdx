...
Stratus is provider-agnostic at its core. The `stratus/core` package defines a `Model` interface that any LLM provider can implement. Azure is the built-in implementation, but you can plug in OpenAI, Anthropic, local models, or any other provider.

- **Two methods** — `getResponse()` for non-streaming, `getStreamedResponse()` for streaming
- **Standard types** — `ChatMessage[]` input, `ModelResponse` output
- **Full compatibility** — Custom models work with all SDK features (tools, handoffs, guardrails, sessions, tracing)
...
export class OpenAIModel implements Model {
  ...
  async *getStreamedResponse(
    ...
    // See AzureChatCompletionsModel source for a complete SSE implementation.
...
<Callout type="info">
The built-in `AzureChatCompletionsModel` is a reference implementation. Use its source code as a guide for building your own — it covers SSE parsing, retry logic, abort signal handling, and error mapping.
</Callout>
...
- [Tools](/docs/tools) — Give your custom model tool-calling capabilities
- [Streaming](/docs/streaming) — Stream responses from any model provider
- [Sessions](/docs/sessions) — Multi-turn conversations with persistent history
- [Tracing](/docs/tracing) — Trace model calls for observability and debugging
