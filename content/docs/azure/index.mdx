...
Both models automatically retry on **429 (rate limit)** responses and **network errors** (timeouts, connection resets, DNS failures). Retries are transparent to the caller — the `AbortSignal` from `RunOptions.signal` still propagates through, so timeouts work across retries.
...
3. **Exponential backoff with jitter** — `1s × 2^attempt + random(0–1s)`, capped at 30s. Used when no headers are present.

Retries are logged via `console.warn` with the wait duration and attempt count.

<Callout type="info">
`AzureResponsesModel` also retries SSE-level rate limits — when the HTTP response is 200 but the stream contains a `too_many_requests` error event before any content has been yielded.
</Callout>
