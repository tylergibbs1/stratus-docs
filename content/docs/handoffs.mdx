...
<Callout type="info">
Handoffs can be blocked by `beforeHandoff` hooks returning `{ decision: "deny" }`. See [Hooks - Permission Control](/docs/hooks#permission-control).
</Callout>
...
Handoffs **transfer control** - the child takes over. Subagents **delegate and return** - the parent keeps control. See [Subagents](/docs/subagents) for the delegation pattern.
</Callout>
