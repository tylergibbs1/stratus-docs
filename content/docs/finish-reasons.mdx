...
Every model response includes a `finishReason` that tells you why the model stopped generating. The run loop inspects this value to decide what happens next -- whether to execute tool calls, return a result, or throw an error.
...
Finish reasons and errors are different categories. A finish reason is part of a successful model response. An error means something went wrong and no usable response was produced.
