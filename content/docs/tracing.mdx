...
Stratus includes an opt-in tracing system that records spans for model calls, tool executions, handoffs, and guardrails. Tracing uses `AsyncLocalStorage` for zero-overhead when inactive.
...
| `subagent` | A [sub-agent](/docs/subagents) execution (includes child agent name) |
