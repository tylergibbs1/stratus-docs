...
Stratus provides three ways to execute an agent. All three handle the full tool loop, guardrails, hooks, and tracing automatically.

- **`run()`** -- Execute and return the final result. Best for server-side tasks where you don't need real-time output.
- **`stream()`** -- Execute and yield events as they arrive. Best for real-time UIs and CLIs.
- **`prompt()`** -- One-shot convenience function. Creates a session, runs a single turn, and returns the result.
...
If the agent has an `outputType`, `finalOutput` contains the parsed and validated object. See [Structured Output](/docs/structured-output) for details.
...
| `lastAgent` | `Agent` | The agent that produced the final response (differs from the entry agent after a [handoff](/docs/handoffs)) |
| `finishReason` | `string?` | The model's finish reason from the last call (`"stop"`, `"tool_calls"`, etc.) |

`UsageInfo` includes `promptTokens`, `completionTokens`, `totalTokens`, and optional `cacheReadTokens` and `cacheCreationTokens` fields.
...
<Callout type="info">
`prompt()` creates a temporary session under the hood. For multi-turn conversations, use [`createSession()`](/docs/sessions) instead.
</Callout>
...
```ts
import type { ChatMessage, ContentPart } from "stratus/core";
...
See [Sessions](/docs/sessions) for the full API, including save/resume/fork and `Symbol.asyncDispose`.
