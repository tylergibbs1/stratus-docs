...
- **One framework, two swappable backends** — [Chat Completions and Responses API](/azure) through the same interface. Switch with one line.
- **No more 404 config spiral** — auto-endpoint detection for standard, foundry, and full URLs. Built-in retry with exponential backoff and unified [content filter errors](/errors).
...
- **Client-side agent state** — no server-side threads or opaque IDs. [Save, resume, and fork](/sessions) conversations with portable snapshots.
- **Budget enforcement built in** — set a [dollar limit](/usage-tracking) and the run stops before you get a surprise bill. Cost estimation accounts for cached and reasoning tokens.
