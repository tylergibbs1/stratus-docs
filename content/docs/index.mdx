...
- **One framework, two swappable backends** — `AzureChatCompletionsModel` and `AzureResponsesModel` implement the same [`Model`](/azure) interface. Switch with one line, zero agent code changes.
- **No more 404 config spiral** — explicit `deployment` param, auto-endpoint detection for standard/foundry/full URLs, built-in retry with exponential backoff, and a unified [`ContentFilterError`](/errors) so Azure's inconsistent error shapes don't break your agent loop.
