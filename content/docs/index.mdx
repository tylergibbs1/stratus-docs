...
- **Tool calling requires a manual loop.** Stratus handles the entire [tool loop](/docs/guides/agentic-tool-use) - one `run()` call with parallel execution, validation, and error recovery.
- **No agent abstraction.** Stratus gives you [composable agents](/docs/agents) with instructions, tools, guardrails, and handoffs in a single config object.
- **Basic streaming.** Stratus provides typed [stream events](/docs/streaming) - `content_delta`, `tool_call_start`, `tool_call_done` - with accumulation and a final `RunResult`.
- **Opaque content filter errors.** Stratus throws a typed [`ContentFilterError`](/docs/errors) instead of Azure's buried `inner_error.content_filter_results`.
- **No multi-agent orchestration.** [Handoffs](/docs/handoffs), [subagents](/docs/subagents), [guardrails](/docs/guardrails), and [hooks](/docs/hooks) are first-class.
- **Responses API support.** Both [Chat Completions and Responses API](/docs/azure) through the same `Model` interface - swap with one line.
