---
title: Introduction
description: A better TypeScript agent SDK for Azure OpenAI
---

Stratus is a better TypeScript agent SDK for Azure OpenAI.
...
- **Built for Azure, not bolted on** — auto-endpoint detection, Entra ID auth, [content filter errors](/errors) as typed exceptions, and built-in retry. No 404 config spirals, no "which SDK do I use" confusion.
- **Two API backends, one interface** — [Chat Completions and Responses API](/azure) through the same agent, tool, and session code. Start with one, switch with a single line.
- **Multi-agent orchestration** — [handoffs](/handoffs), [subagents](/subagents), [guardrails](/guardrails), and [hooks](/hooks) compose through a single run loop. Hooks can deny or modify tool calls at runtime.
- **Client-side state you control** — [save, resume, and fork](/sessions) conversations as portable JSON snapshots. No server-side threads, no opaque session IDs.
- **Type-safe from schema to output** — Zod schemas drive [tool parameters](/tools), [structured output](/structured-output), and validation. Context types flow through agents, hooks, and guardrails at compile time.
...
import { createSession, tool } from "@usestratus/sdk/core";
import { z } from "zod";
