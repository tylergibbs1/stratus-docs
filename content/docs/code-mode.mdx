...
Runs code using `AsyncFunction` in the current runtime (Bun or Node.js).
...
<Callout type="warn">
`FunctionExecutor` runs code in the same V8 isolate — it is **not** a secure sandbox. For production use with untrusted code, implement a custom `Executor` using isolated-vm, containers, or Cloudflare Workers.
</Callout>
...
- `FunctionExecutor` is not a secure sandbox — it runs in the same process
