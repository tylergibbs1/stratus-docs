...
When the output token count crosses `compact_threshold`, the API automatically compacts the context and emits a compaction item. On subsequent turns the compaction item carries forward essential context using fewer tokens.
